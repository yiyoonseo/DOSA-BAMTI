      ...
      <div className="bg-[#F0F2F5] rounded-[20px] p-4 relative">
        ...
        {initialData ? (
          ...
          <button
            ...
            className={`flex items-center gap-1 b-10-semi p-[8px] rounded-full font-bold transition-all cursor-pointer ${
              isAiNoteOpen
                ? "bg-[#CD3F3F] text-white hover:bg-[#b03535] shadow-md border border-[#CD3F3F]" //여기 추후 수정
                : "border-ai-gradient text-main-1 hover:bg-blue-50"
            }`}
          >
        ...
        {otherAttachments.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-3 mt-1">
            {otherAttachments.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-1 bg-white border border-gray-200 px-2 py-1 rounded-md text-[10px] text-gray-600 shadow-sm"
              >
        ...
        <div className="flex justify-between items-end mt-2 relative">
                  ...
                  <div className="p-2 space-y-1">
                    {categoryList.map((cat, idx) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 text-xs text-gray-900 hover:bg-[#EDF2F6] rounded-lg transition-colors"
                      >
                        {cat}
                      </button>
                    ))}
                    <button
                      ...
                      className="w-full text-center mt-1 px-3 py-2.5 text-xs bg-[#EDF2F6] text-gray-600 hover:bg-[#D0D0D0] rounded-lg transition-colors"
                    >
                      카테고리 추가
                      <Plus
                        size={14}
                        className=" pb-1 pl-1 inline-block text-gray-600"
                      />
                    </button>
                  </div>
                </div>
          ...
          <button
            onClick={handleSave}
            className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors shadow-md ${content.trim() || attachments.length > 0 ? "bg-[#374151] text-white hover:bg-black" : "bg-gray-300 text-white cursor-not-allowed"}`}
          >
